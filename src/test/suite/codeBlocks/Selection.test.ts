...
suite("Selection", function () {
    ...
    suite("TSX", function () {
        ...
        let selectionAt: UpdateRunner = undefined;
        this.beforeAll(async () => {
            const tsx = await Installer.loadParser(parsersDir, "tree-sitter-typescript", "tree-sitter-tsx");
            assert.ok(tsx);
        ...
        test("Update selection previous/next", async () => {
            ...
            expect(await selectionAt(text, ["parent", "add-previous", "remove-next"])).to.equal("<li>1</li>");
            ...
            expect(await selectionAt(text, ["parent", "add-next", "remove-previous"])).to.equal("<li>3</li>");
        });
    });
});
